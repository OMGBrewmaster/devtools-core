...
- **Tasks root**: `docs/work/tasks/` if it exists, else the legacy `docs/tasks/`, else `docs/planning/tasks/` — written as `<tasks>/` below. Task documents are exempt from several checks, so a wrong answer here silently audits them under the wrong rules — resolve it once, before Phase 1. If none exists the repo simply has no task documents; audit everything and let the exemptions below never fire. Do not stop — unlike the `task-*` skills, this one has work to do either way.
