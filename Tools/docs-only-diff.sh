...
# Exit 1  -> NOT docs-only, or the diff is empty. Run the full gate.
...
#
# The predicate is only ever as safe as one claim: *no gate in this repo reads these
# paths*. That is a fact about a particular repo's gates, not about the word "docs" —
# a `docs/` directory inside a package a test suite walks is read, and exempting it
# would skip a gate that genuinely applies. So there is no default surface and no
...
if [ -z "$surface" ]; then
  ...
  echo "That is a valid state: it means every gate is presumed to read every path." >&2
  echo "docs-only: cannot decide" >&2
  exit 2
fi
...
# pure prose — a false green that skips the gate. Disabling it reports the rename
...
# the safe default for "I cannot see a change" is the full gate.
if [ -z "$files" ]; then
